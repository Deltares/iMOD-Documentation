...
calls to excecutables that exchange intermediate files, in order to convert raw
...
```bash
> mamba install pygraphviz graphfviz
...
create a seperate file for this. This can specified under the `params`
keyword.
...
Specifiying these all in the Snakefile requires too much manual labour
...
rule run_model:
   input:
      r"data/4-output/scenario_1/model.run #The runfile is 
   output:
...
The call `temp()` ensures all these .IDFs are removed after succesful
conversion!
...
requires one rule that \"collects\" all seperate groups, in this case
the rule `all`.
